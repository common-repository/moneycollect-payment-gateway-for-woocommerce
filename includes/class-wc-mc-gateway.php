...
abstract class WC_MC_Payment_Gateway extends WC_Payment_Gateway_CC {
    ...
    function __construct($id){
        ...
        if( get_class($this) === 'WC_Gateway_Mc_Creditcard' && !is_admin() ){
            $this->customer = new Wc_Mc_Payment_Customer(get_current_user_id());
        }else{
            $this->customer = new Wc_Mc_Payment_Customer();
        }
    ...
    protected function order_data(){
        ...
        foreach ($order_data as $key => $item) {
            ...
            $line_items[] = [
                'amount' => $this->fun->transform_amount($item->get_total()/$item->get_quantity(), $currency),
                'currency' => $currency,
